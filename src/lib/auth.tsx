...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login: AuthContextType["login"] = async (email, password, remember = true) => {
    ...
    const validHardcoded = e === HARDCODED_USER.email.toLowerCase() && password === HARDCODED_USER.password;
    const validSignup = users[e] && users[e] === password;
    if (!validHardcoded && !validSignup) {
  ...
  const signup: AuthContextType["signup"] = async (email, password) => {
    ...
    const users = getStoredUsers();
    if (users[e] || e === HARDCODED_USER.email.toLowerCase()) {
